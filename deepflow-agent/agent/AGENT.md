...
|--------|-------------------|-----------------------------------------|
| `zip`  | `ZipDestination`  | Accumulates per-thread, writes zip on close |

The destination constructor receives a `Map<String, String>` config with
`session_dump_location` and `session_id`. Future destinations (e.g. HTTP)
would receive their own parameters through the same map.
