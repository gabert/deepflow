#!/usr/bin/env bash
#
# Builds the project, starts the Spring Boot demo with the agent,
# exercises the API with two separate users (two HTTP sessions),
# prints the trace output, and tears down.
#
set -euo pipefail
...
cd "$SCRIPT_DIR"
mvn spring-boot:run \
    -Dspring-boot.run.jvmArguments="-javaagent:../agent/target/deepflow-agent.jar=config=./deepagent.cfg" \
    > /dev/null 2>&1 &
MVN_PID=$!
