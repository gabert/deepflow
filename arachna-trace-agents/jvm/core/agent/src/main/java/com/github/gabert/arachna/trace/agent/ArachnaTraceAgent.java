...
public class ArachnaTraceAgent {
    private static final String AGENT_PACKAGE = "com.github.gabert.arachna.trace.agent";
    ...
    public static void premain(String agentArgs,
        ...
        // Inject bootstrap classes BEFORE ArachnaTraceAdvice or any executor advice
        ...
        System.out.println("[ArachnaTrace] Agent attached — destination=" + agentConfig.getDestination()
    ...
    private static void injectBootstrapClasses(Instrumentation instrumentation) {
        ...
        try {
            File tempJar = File.createTempFile("arachna-trace-bootstrap", ".jar");
            tempJar.deleteOnExit();

            try (JarOutputStream jos = new JarOutputStream(new FileOutputStream(tempJar))) {
                ClassLoader cl = ArachnaTraceAgent.class.getClassLoader();
            ...
            Class<?> injectedClass = Class.forName(
                    "com.github.gabert.arachna.trace.agent.bootstrap.RequestContext", true, null);
            ...
            System.err.println("[ArachnaTrace] Warning: failed to inject bootstrap classes. "
    ...
    private static void installExecutorInstrumentation(Instrumentation instrumentation) {
        ...
        System.out.println("[ArachnaTrace] Executor instrumentation installed (request ID propagation)");
    }
